...
def test_e2e_with_sample_pdf(monkeypatch):
    ...
    with mock.patch("agent.graph.extract_events") as mock_extract:
        ...
        # Updated mock to match what's actually imported in agent.graph
        with mock.patch("agent.graph.deduplicate_single_date_group") as mock_dedup:
            mock_dedup.side_effect = lambda date, events: events  # Pass through
